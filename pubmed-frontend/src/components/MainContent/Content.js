...
const Content = ({ article }) => {
  ...
  let year, month, day;
  if (
    article.pubDate.elements.length === 3 ||
    article.pubDate.elements.length > 3
  ) {
  ...
  if (authorsList.some((type) => type.name === "ForeName"))
    foreName = authorsList.find((author) => author.name === "ForeName")
      .elements[0].text;
  if (authorsList.some((type) => type.name === "Initials"))
    initials = authorsList.find((author) => author.name === "Initials")
      .elements[0].text;
  if (authorsList.some((type) => type.name === "LastName"))
    lastName = authorsList.find((author) => author.name === "LastName")
      .elements[0].text;
  ...
  return (
    <div style={{ borderTop: "solid", paddingTop: 25, borderWidth: 1 }}>
      <div style={{ marginLeft: 30, marginRight: 30 }}>
        <a
          style={{ fontWeight: 400, color: "whitesmoke" }}
      ...
      <div
        style={{ fontSize: 20, marginTop: 5, marginLeft: 30, marginRight: 30 }}
      >
      ...
      <Collapse in={expanded} timeout="auto" unmountOnExit>
        <div style={{ marginLeft: 30, marginRight: 30 }} id="abstract-content">
          {abstract.map((part) => {
            ...
            if (part.attributes) {
              ...
              return (
                <div key={Math.floor(Math.random() * 100000000000000)}>
                  <p key={Math.floor(Math.random() * 100000000000000)}>
                    {attributes}
                  </p>
                  <p key={Math.floor(Math.random() * 100000000000000)}>
                    {abstractText}
            ...
            return (
              <div key={Math.floor(Math.random() * 100000000000000)}>
                {abstractText.map((text) => (
                  <text key={Math.floor(Math.random() * 100000000000000)}>
                    {text}
                  </text>
                ))}
              </div>
